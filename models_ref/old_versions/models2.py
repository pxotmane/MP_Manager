# -*- coding: utf-8 -*-
"""
Modèles Django -- app "procurement"
Générés à partir de MODELS_TABLES.xlsx (feuilles : FICHE_MARCHE, NANTISSEMENT)

ATTENTION : FICHE_MARCHE semble recouper votre modèle "Marche" existant
(AppelOffres, Marche, Soumission, CautionBancaire). Cette proposition crée un
modèle distinct FicheMarche -- à vous de décider si vous voulez fusionner ces
champs dans Marche, ou garder FicheMarche comme fiche financière détaillée
liée en OneToOne à Marche.
"""
...
class FicheMarche(models.Model):
    ...
    type_marche = models.CharField(
        max_length=30, choices=TypesMarche.choices, verbose_name="Type de marché"
    )
    ...
    rabais = models.DecimalField(
        ...
        verbose_name="Rabais",
        help_text=(
            "Montant ou pourcentage ? La source ne précise pas -- ajustez decimal_places "
            "si c'est un taux (ex. max_digits=5, decimal_places=2 pour un pourcentage). "
            "Même remarque pour 'majoration' ci-dessous."
        ),
    )
    majoration = models.DecimalField(max_digits=5, decimal_places=2, default=0, verbose_name="Majoration")
